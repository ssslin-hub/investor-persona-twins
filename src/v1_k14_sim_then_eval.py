"""Fresh V1 K=14 simulation + 5-run gpt-5 eval, parameterized by rerun label.
Used to launch multiple fresh sims in parallel, each followed by 5 evals.
"""
...
def simulate(out_dir):
    ...
    for name in ALL_11:
        ...
        if name in COLD:
            p = os.path.join(ROOT, 'data_auto', 'final_personas', '_fallback.json')
        else:
            p = os.path.join(ROOT, 'data', 'personas', f"{name.replace(' ','_')}.json")
        ...
        per_analyst[name] = {'n_actual': len(actuals), 'predictions': pred,
                              'persona_source': 'v1' if name not in COLD else 'auto-fallback'}
        total += len(pred.get('predicted_questions', []))
        print(f"  {name:25s} K={len(pq)}")
    summary = {'K': 14, 'source': 'v1', 'n_analysts_scored': len(per_analyst),
...
def main():
    ...
    ap.add_argument('--label', required=True, help='e.g. rerun3')
    ap.add_argument('--n-eval-runs', type=int, default=5)
    args = ap.parse_args()
    out_dir = os.path.join(ROOT, 'data_auto', f'final_eval_14q_v1_{args.label}')
    os.makedirs(out_dir, exist_ok=True)
    summary_path = os.path.join(out_dir, 'summary.json')
    if not os.path.exists(summary_path):
        print(f"=== SIMULATE {args.label} ===")
        simulate(out_dir)
    else:
        print(f"=== SIMULATE {args.label} (cached) ===")
    var_dir = os.path.join(out_dir, 'variance')
