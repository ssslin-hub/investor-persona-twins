...
Q&A history in operator order. K=10 per turn.
...
OUT_DIR = os.path.join(DATA_AUTO, "final_eval_conv_10q")
LOG_DIR = os.path.join(OUT_DIR, "logs")
os.makedirs(LOG_DIR, exist_ok=True)

SIM_PROMPT = os.path.join(PROMPTS, "simulate_questions_conversational_10q.md")
...
def main() -> None:
    ...
    for t in queue:
        ...
        if len(pred.get("predicted_questions", [])) > 1:
            pred["predicted_questions"] = pred["predicted_questions"][:10]
    ...
    summary = {
        "mode": "conversational", "K_per_turn": 10, "n_turns": len(per_turn),
