"""Generic parallel K-simulator: one LLM call per twin produces K ranked questions.
Reuses persona-loading from rerun_20q pattern; the K value selects the prompt file
`prompts/simulate_questions_{K}q.md` (must exist).

Usage: python3 src/rerun_kq.py --K <int> --source v1|auto

Output: data_auto/final_eval_{K}q_{source}/summary.json
"""
...
def run(K: int, source: str) -> None:
    ...
    out_dir = os.path.join(DATA_AUTO, f"final_eval_{K}q_{source}")
    ...
    for name in ALL_11:
        ...
        if persona is None:
            ...
            continue
        sp = build_simulator_prompt(sim_tpl, persona, mgmt)
        out = call_llm(sp, expect_json=True,
        ...
        if len(pq) > K:
            ...
            print(f"  ! {name}: simulator returned {len(pq)} questions (expected {K})")
        per_analyst[name] = {
