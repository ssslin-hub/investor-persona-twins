"""K=5 ablation: re-simulate TEST with EXACTLY 5 predicted questions per
...
Mirrors src/rerun_1q.py structure exactly; differences:
  - prompts/simulate_questions_5q.md  (K=5 instead of K=1)
  - --source v1|auto subcommand selects persona directory
  - defensive cap to 5 if simulator returns more

Outputs:
  data_auto/final_eval_5q_v1/{summary.json, b4.json, logs/}
  data_auto/final_eval_5q_auto/{summary.json, b4.json, logs/}
"""
...
SIM_PROMPT = os.path.join(PROMPTS, "simulate_questions_5q.md")
...
def run(source: str) -> None:
    ...
    out_dir = os.path.join(DATA_AUTO, f"final_eval_5q_{source}")
    ...
    print(f"=== K=5 rerun (source = {source}) ===")
    ...
    for name in ALL_11:
        ...
        if len(pq) > 5:
            pred["predicted_questions"] = pq[:5]
        elif len(pq) < 5:
            print(f"  ! {name}: simulator returned {len(pq)} questions (expected 5)")
    ...
    print(f"B2 aggregate: binary_rate={binary/max(1,len(scored)):.3f} ({binary}/{len(scored)}); avg={avg:.3f}")
